...
function sumOfCodes(str) {
  if (!str) return 0;
  return str.split('').reduce((acc, letter) => acc + letter.charCodeAt(0), 0);
}
...
function countVowels(str) {
  ...
  const result = str.split('').filter((letter) => vowels.includes(letter));
...
function isPalindrome(str) {
  ...
  const reverseStr = clearedStr.split('').reverse().join('');
  return clearedStr === reverseStr;
}
